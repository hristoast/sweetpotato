...
class ScreenNameAlreadyInUseError(SweetpotatoIOErrorBase):
    """Raised when the configured screen name is already in use."""
    pass
...
def is_screen_name_a_dupe(screen_name):
    # TODO: we need to know when a screen of the same name is already in use
    screen_started = is_screen_started(screen_name)
    if screen_started:
        running_screen_name = screen_started.split('.')[1]
        if running_screen_name == screen_name:
            raise ScreenNameAlreadyInUseError('The screen name "{}" is already in use!'.format(screen_name))
...
def arg_parse(argz):
    ...
    # if running:
    #     try:
    #         is_screen_name_a_dupe(settings.screen_name)
    #     except ScreenNameAlreadyInUseError as e:
    #         error_and_die(e)
